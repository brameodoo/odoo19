...
class SgsCustodian(models.Model):
    ...
    # Relación directa con hr.employee para evitar duplicar esfuerzos
    employee_id = fields.Many2one(
        'hr.employee', 
        string='Empleado Relacionado', 
        tracking=True, 
        index=True,
        help="Enlace con el registro oficial de empleados de la empresa."
    )
    ...
    # Autorellenar datos desde el empleado seleccionado al cambiar la relación
    ...
    def _compute_portal_url(self):
        ...
        for rec in self:
            ...
            phone_clean = ''.join(ch for ch in (rec.phone or '') if ch.isdigit())
            if phone_clean and rec.portal_url:
                ...
                rec.whatsapp_url = 'https://wa.me/%s?text=%s' % (phone_clean, msg.replace(' ', '%20'))
            else:
                rec.whatsapp_url = ''
...
class SgsVehicle(models.Model):
    ...
    def _uppercase_vehicle(self):
        for rec in self:
            if rec.plate: rec.plate = rec.plate.upper()
            if rec.brand: rec.brand = rec.brand.upper()
            if rec.model: rec.model = rec.model.upper()
...
class SgsRouteService(models.Model):
    ...
    def action_approve(self): self.write({'status': 'approved'})
    def action_reject(self): self.write({'status': 'rejected'})
    def action_pending(self): self.write({'status': 'pending'})
...
class SgsTollLine(models.Model):
    ...
    def _check_amount(self):
        for rec in self:
            if rec.amount <= 0: raise ValidationError(_('El monto de la caseta debe ser mayor a cero.'))
